...
class database():
    ...
    cnx = mysql.connector.connect(
        user='root', password='root', host='127.0.0.1', database='ecofablab')

    cursor = cnx.cursor(buffered=True)

    def justCheckedIn(self, memberId):
        qy_present = "SELECT COUNT(*) FROM presence WHERE arrival_time > DATE_SUB(NOW(), INTERVAL 1 MINUTE) AND member_id = '{}'".format(memberId)
    ...
    def isMember(self, memberId):
        qy_find_member = "SELECT COUNT(*) FROM member WHERE member_id = '{}'".format(
            memberId)
    ...
    def writePresence(self, memberId):
        if self.justCheckedIn(memberId):
            print("Member " + str(memberId) +
                  " has already checked in, will do nothing")
        else:
            arrival_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

            qy_find_presence = "SELECT row_id FROM presence WHERE date(arrival_time) = '{}' AND departure_time IS NULL AND member_id = '{}'".format(
                datetime.now().strftime('%Y-%m-%d'), memberId)
            ...
            if self.cursor.rowcount > 0:
                ...
                for row_id in row_ids:
                    qy_update_presence = "UPDATE presence SET departure_time = '{}' where row_id = {};".format(
                        datetime.now().strftime('%Y-%m-%d %H:%M:%S'), row_id[0])
            ...
            # new presence
                ...
                qy_add_presence = "INSERT INTO presence (member_id, arrival_time) VALUES ('{}', '{}')".format(
                    memberId, arrival_time)
                ...
                print(
                    str(memberId) + " is NOT a member and will not be added to the presence tabel")
